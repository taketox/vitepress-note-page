...
- 缺乏包管理系统。
  ...
  > 这导致JavaScript应用中基本没有自动加载和安装依赖的能力。
...
![An image](/img/nodejs/moudle/01.png)
...
在模块中，上下文提供`require()`方法来引入外部模块。对应引入的功能，上下文提供了`exports`对象用于导出当前模块的方法或者变量，并且它是唯一导出的出口。在模块中，还存在一个module对象，它代表模块自身，而`exports`是module的属性。在Node中，一个文件就是一个模块，将方法挂载在`exports`对象上作为属性即可定义导出的方式：
...
![An image](/img/nodejs/moudle/02.png)
